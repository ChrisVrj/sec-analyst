...
- **🔴 The tradeable-universe gate — read before touching priority routing.** Chris trades **exchange-listed income securities at retail denomination**: $25-par preferreds and depositary shares, baby bonds, exchange-traded debt, CEF/BDC shares. He does **not** trade common stock, and does **not** trade **$1,000-par institutional paper** (senior notes, global notes, MTNs) *even when NYSE-listed*. **A $1,000 denomination is the single clearest tell that a filing is irrelevant to him.**

  On 2026-08-07 the urgent channel produced 8 pings; **6 were wrong and all 6 were tier 2 "LISTING" blocks** — AT&T and BNY Mellon $1,000-par senior notes, INN and CSWC common-stock ATMs, an OCFC 10-Q whose own body said `Listing: UNLISTED`, and an AOD shelf with nothing priced. The two correct ones were a *redemption* (MBIN, at "$25 per depositary share") and a *tender/exchange offer* (SQFT, 5.5 common per Series D preferred — a real pair trade, since SQFT traded ~$2.10 against SQFTP ~$6.10).

  Two compounding causes: the prompt treated *any* NYSE/NASDAQ listing as priority 2, and the model reported **the issuer's common ticker as the offered security's symbol** — 'NYSE SYMBOL "T"' for AT&T's notes, `"BNY"` for BNY Mellon's. An exchange-traded preferred has its own ticker (`SQFT` common vs `SQFTP` preferred).

  Fix is two-layer, because the model demonstrably gets this wrong: the prompt now defines the tradeable universe up front with explicit ticker discipline, **and** `_is_tradeable_new_issue()` gates tier 2 in code on the `Product` / `Par` / `Listing` fields the model already writes. Demotes on: `Listing: UNLISTED`, `Par: $1,000`, `Product: common stock`, or the absence of any preferred/depositary/baby-bond/$25-par signal. **Only tier 2 is gated** — tiers 1, 3 and 4 are not, because a missed redemption or tender costs far more than a spurious one. Tier 2 fails safe to routine, so a demoted filing still reaches `#sec-filings`. All eight real posts are locked into `test_pipeline.py` as fixtures; add the next bad ping there.
...
- **The calendar-form gate (added 2026-08-10) — the second false-ping class.** On 2026-08-10 two Equitable Holdings posts pinged the urgent channel with **no highlight block at all**: `🚨 EQH | ARS` ("annual report to stockholders for fiscal year 2025") and `⚠ EQH | DEF 14A` ("annual meeting to vote on director elections, auditor ratification, and say-on-pay"). Neither discloses anything tradeable. Both routed purely on `classify_priority()`'s **weak signal — the line-1 emoji**, which the model picks freely and had simply reached for. (The `⚠ CHMI | 8-K` posted minutes later in the same channel *was* correct: a real merger with preferreds converting, carrying a real `## ⚠️ M&A — CHANGE OF CONTROL` block.)

  The aggravating factor is form frequency: **every issuer files a proxy and an annual report every year**, so an occasional emoji slip on those forms becomes a steady drip of proxy-season pings. So `_CALENDAR_FORMS` disables *only the emoji signal* for forms that exist to satisfy a filing calendar — proxies and information statements, `ARS`, 10-K/10-Q/20-F/40-F, `NT` late notices, fund periodics (`N-CSR`, `N-PORT`, `N-CEN`, …), and ownership/holdings snapshots. The header signal is untouched everywhere. Three deliberate properties: the form type comes from **the EDGAR payload, not the model** (`classify_priority(summary, form_type)`); merger-vote proxies (`DEFM14A` / `PREM14A`) are absent from the set, and even a plain `DEF 14A` still pings if it quotes change-of-control terms in a `##` block; and it is a **denylist, so an unrecognised form keeps the old permissive behaviour**. MBIN's genuine redemption was disclosed inside a **10-Q** — a gated form — and survives on its header; that fixture now runs with its real form type in `test_pipeline.py` precisely to keep proving it.

  A demoted rule now `continue`s to the next tier instead of returning 0, so a weak hit on one tier can never suppress a header-backed hit on another (a 10-K with a stray 🚨 *and* a real `## ⚠️ M&A` block classifies tier 3).
...
8. **One filing, one decision** (2026-09-01, found while investigating NFJ's unposted `SC TO-I`). The feed emits **one entry per role**, each with its own CIK in the href. `poll_once()` iterated entries and did `seen.add(accession)` on the first role whose CIK missed the watchlist — which then made the matching role hit `if accession in seen: continue`. The watchlist company's own filing was discarded before it was ever examined.

   Measured against a 600-entry feed sample: of **119 multi-role filings touching the watchlist, 114 had the watchlist CIK in the second entry** — 85 bank-shelf `424B2`s (guarantor behind the funding subsidiary), four `144`s where the watchlist name is the Subject, and a **Saba Form 4 on ECF**, which is precisely the activist surveillance in §3. The Sep 2026 accession-level dedupe in `fetch_recent_filings()` made it worse by discarding the role before `poll_once()` could see it at all.

   Fixed in two places that have to agree: the feed walk deduplicates on **`(accession, cik)`**, and `poll_once()` groups by accession and picks the watchlist role before deciding. Both have fixtures.

9. **A failed document fetch was a permanent, silent loss.** `poll_once()` did `seen.add(accession)` *before* checking whether `fetch_filing_documents()` returned anything, so one 503 or read timeout retired the filing forever with a WARNING in a 7-day artifact. EDGAR fails intermittently under ordinary load — several 503s and read timeouts were hit while testing this very session, at a request rate far below the poller's — and an index page is not reliably complete in the seconds after the filing appears in the feed, which is when a 15-second poll first asks for it. Now retried across cycles (`MAX_FETCH_ATTEMPTS`, counter persisted in `fetch_failures.json`, deliberately not cached between runs since losing it only costs extra retries), and giving up posts a ❌ to Discord with the EDGAR link via `notify_failure()`.

10. **Tender offers had no entry in `triage.py`.** They are tier 4 in `URGENT_RULES`, but the module scored a Schedule TO at 0, so one paged only if the model happened to write a `## 🔁 TENDER` block — the same dependence on wording that lost CLM. `TENDER_FORMS` now pages on form type alone. **Amendments page too**, unlike `CAPITAL_RAISE_FORMS`: an amended Schedule TO is usually a price increase or an extension, both terms of a live offer rather than an announcement already made.

    ⚠ **None of these is confirmed as the cause of the NFJ miss.** Run #2036 was polling 14:44–18:00 Sofia and the filing was accepted at 14:54; NFJ is on the watchlist, the filing is single-role, `fetch_filing_documents()` returns 91k characters for it today, and the prefilter keeps it. The run log needs authentication to read and the artifact has since expired, so the cause was never pinned down. Items 8-10 are the bugs that *were* provable, and each produces exactly this symptom — a filing that exists, a poller that was running, and nothing posted.

`test_pipeline.py` carries 151 checks after the merge, including the first coverage of `edgar_poller.py` (feed paging, role-entry dedupe, horizon stop) — previously listed in §9 as the most valuable untested area.
