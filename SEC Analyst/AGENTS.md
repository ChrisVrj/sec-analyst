...
- **DST transition isn't automated.** The bash window-check's UTC boundaries are hardcoded for EEST (summer). They need a manual +60-minute edit around the last Sunday of October and again around the last Sunday of March. Consider computing the Sofia UTC offset from the date itself (e.g. Python's `zoneinfo.ZoneInfo("Europe/Sofia")`, which `edgar_poller.py` already uses for its own `ET_ZONE`-style logic, could be adapted) instead of hardcoded minute constants in bash.
...
- **No automated tests.** Given how many of the historical bugs were "the document extraction silently grabbed the wrong content" or "the LLM silently produced a useless response," some kind of regression fixture (a handful of saved real EDGAR filings with known-good expected extracted text, or even just a smoke test that `extract_document_urls()` returns a non-empty list for a known accession) would catch future regressions faster than re-discovering them via days of Discord silence.
